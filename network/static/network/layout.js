...
function login_popup(action) {
    ...
    if(action === 'like') {
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Поставьте лайк на пост, чтобы порадовать человека, который выложил пост';
    }
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Прокомментируйте, чтобы присоединиться к разговору';
    }
    else if(action === 'save') {
        document.querySelector('.icon-div').innerHTML = `
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Сохраните пост, чтобы посмотреть его позже';
    }
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Следите за людьми, которые вас вдохновляют';
    }
}
...
function follow_user(element, username, origin) {
    ...
    .then(() => {
        if(origin === 'suggestion') {
            element.parentElement.innerHTML = `<button class="btn btn-success" type="button" onclick="unfollow_user(this,'${username}','suggestion')">Отписаться</button>`;
        }
        else if(origin === 'edit_page') {
            element.parentElement.innerHTML = `<button class="btn btn-success float-right" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Отписаться</button>`;
        }
...
function unfollow_user(element, username, origin) {
    ...
    .then(() => {
        if(origin === 'suggestion') {
            element.parentElement.innerHTML = `<button class="btn btn-outline-success" type="button" onclick="follow_user(this,'${username}','suggestion')">Подписаться</button>`;
        }
        else if(origin === 'edit_page') {
            element.parentElement.innerHTML = `<button class="btn btn-outline-success float-right" onclick="follow_user(this,'${username}','edit_page')" id="follow-btn">Подписаться</button>`;
        }
        else if(origin === 'dropdown') {
            ///////////////////////////////////////////////////////////////////////////////////////////
        }
